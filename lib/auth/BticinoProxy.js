...
class BticinoProxy {
  ...
  /**
   * Mask sensitive parameters (such as passwords, secrets, tokens) in log output.
   * @private
   * @param {string} str - Raw string or body content
   * @returns {string} Sanitized string with sensitive values replaced by masked placeholders
   * @example
   * const safe = this._sanitizeLog('logonIdentifier=user@email.com&password=SecretPassword123');
   */
  _sanitizeLog(str) {
  ...
  async start() {
    if (this.server) {
      this._log('Proxy already started');
      return;
    }
      ...
      this.server.listen(this.port, this.host, (err) => {
        if (err) {
          if (err.code === 'EADDRINUSE') {
            this._warn(`Port ${this.port} busy, trying ${this.port + 1}`);
            this.port++;
            this.start().then(resolve).catch(reject);
          } else {
            this._error('Failed to start proxy:', err.message);
            reject(err);
          }
        } else {
          this._log(`🔍 BTicino Proxy running on http://${this.host}:${this.port}`);
          resolve();
        }
      });
