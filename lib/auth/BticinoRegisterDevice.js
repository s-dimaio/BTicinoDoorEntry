...
async function BticinoRegisterDevice(auth, api, options = {}) {
  ...
  if (gatewayId) {
    ...
    const existing = sipAccounts.find(acc =>
      String(acc.clientId) === String(sipClientId) ||
      (opts.clientName && acc.clientName === opts.clientName)
    );
    if (existing) {
      _log('SIP account already exists (clientId:', existing.clientId, ', name:', existing.clientName, '), reusing existing account');
      if (typeof api.setSipClientId === 'function') {
        api.setSipClientId(String(existing.clientId));
      } else {
        api.sipClientId = String(existing.clientId);
      }
      try {
        emitter.emit('registered', { gatewayId, sipClientId: existing.clientId, result: existing });
      } catch (e) {
        _error('Failed emitting registered event for existing account:', e && e.message);
      }
    } else {
  ...
  if (gatewayId && plantId) {
    try {
      ...
      if (plantInfo) {
        ...
        const shouldProvision = registrationHappened || (opts && opts.forceCertificates === true) || !!existing;
        if (!shouldProvision) {
