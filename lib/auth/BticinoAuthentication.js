...
const BticinoCertificateManager = require('../certs/BticinoCertificateManager');
const { BticinoSipListener } = require('../sip/BticinoSipListener');
 ...
 *  3. Wait until the mobile custom-scheme redirect is intercepted and extract the authorization code
 *  4. Exchange the authorization code for B2C (and optionally Legrand) tokens, unless disabled
 *  5. Schedule automatic refresh before expiry
 *  6. Emit an event `tokenCreated` whenever a fresh token set is acquired (initial / refresh / forced)
 *  7. Emit an event `tokenRefreshed` ONLY for refresh events (scheduled / forced / immediate)
 *  8. Emit an event `loginUrl` when an interactive login is required. Payload: an object
...
class BticinoAuthentication extends EventEmitter {
    ...
    constructor(options = {}) {
        ...
        this._sipListener = null; // Track active SIP listener to prevent duplications

        // Initialize certificate manager with matching options
        this.certificateManager = new BticinoCertificateManager(null, {
            autoRenewal: this.opts.autoRefresh, // Mirror token autoRefresh behavior
            renewalSkewSec: this.opts.certificateRenewalSkewSec !== undefined ? this.opts.certificateRenewalSkewSec : (30 * 24 * 60 * 60), // 30 days default
            unrefRenewalTimer: this.opts.unrefRefreshTimer,
            debug: this.opts.debug
        });

        // Forward certificate manager events to this instance
        this.certificateManager.on('certificatesCreated', (certs, meta) => {
            try {
                this.emit('certificatesCreated', certs, meta);
            } catch (e) {
                this._error('emit(certificatesCreated) from manager error:', e && e.message);
            }
        });

        this.certificateManager.on('certificatesRefreshed', (certs, meta) => {
            try {
                this.emit('certificatesRefreshed', certs, meta);
            } catch (e) {
                this._error('emit(certificatesRefreshed) from manager error:', e && e.message);
            }
        });
    ...
    * Ensure the underlying {@link BticinoProxy} is started exactly once.
    ...
    prepareLoginUrl() {
        ...
        const fullUrl = `http://${hostDisplay}:${this.proxy.port}${pathUrl}`;
     ...
     */
    async shutdown() {
        // Perform a graceful shutdown: cancel refresh timer, shutdown certificate manager, and stop proxy.
        try {
        ...
        try {
            if (this.certificateManager && typeof this.certificateManager.shutdown === 'function') {
                this.certificateManager.shutdown();
            }
        } catch (e) {
            this._log('Error shutting down certificate manager:', e && e.message);
        }
    ...
    createApiClient(options = {}) {
        ...
        // Inject the API client into certificate manager so it can perform renewals
        if (this.certificateManager && !this.certificateManager.api) {
            this.certificateManager.api = api;
        }
        return api;
    }

    /**
     * Set current certificates and schedule automatic renewal.
     * Wrapper around certificateManager.setCertificates().
     * @param {string} certPEM PEM-encoded certificate
     * @param {string} privateKeyPem PEM-encoded private key
     * @param {Object} [renewalParams] Parameters required for renewal (ownerId, ownerEmail, deviceId, plantId, gatewayId)
     * @param {Object} [meta] Metadata flags
     * @returns {Object} Enriched certificate object
     * @example
     * auth.setCertificates(certPEM, privateKeyPem, {
     *   ownerId: 'user-uuid',
    ...
    setCertificates(certPEM, privateKeyPem, renewalParams = null, meta = {}) {
        ...
        return this.certificateManager.setCertificates(certPEM, privateKeyPem, renewalParams, meta);
    }

    /**
     * Get current certificates from the certificate manager.
     * @returns {Object|null} Current enriched certificate object or null
     * @example
     * const certs = auth.getCurrentCertificates();
     * if (certs) console.log('Certificate expires at:', certs.expiresAt);
     */
    getCurrentCertificates() {
        if (!this.certificateManager) return null;
        return this.certificateManager.getCurrentCertificates();
    }

    /**
     * Force immediate certificate renewal.
     * Wrapper around certificateManager.forceRenew().
     ...
     */
    async forceRenewCertificates(overrideParams = null) {
        if (!this.certificateManager) {
            throw new Error('Certificate manager not initialized');
        }
        return await this.certificateManager.forceRenew(overrideParams);
    }
    ...
    async registerDevice(options = {}) {
        ...
        const api = this.createApiClient({ debug: opts.debug, sipClientId: opts.sipClientId });
        ...
        // Handle certificates from registerDevice:
        // Instead of forwarding directly, initialize the certificate manager
        // so that consumers receive a unified event signature (enrichedCerts, meta)
        emitter.on('certificatesCreated', (info) => {
            try {
                // Initialize certificate manager with the provisioned certificates
                // This will emit 'certificatesCreated' with proper enrichment and meta
                if (this.certificateManager && info && info.certPEM && info.privateKeyPem) {
                    // Build renewal params from the provisioning info
                    const renewalParams = {
                        ownerId: info.prov?.ownerId || info.ownerId,
                        ownerEmail: info.prov?.ownerEmail || info.ownerEmail,
                        deviceId: info.prov?.deviceId || info.deviceId,
                        plantId: info.plantId,
                        gatewayId: info.gatewayId,
                        plantName: info.prov?.plantName || info.plantName,
                        country: info.prov?.country || 'it'
                    };
                    // setCertificates will emit 'certificatesCreated' via the manager's forwarded event
                    // with proper (certs, meta) signature
                    this.certificateManager.setCertificates(
                        info.certPEM,
                        info.privateKeyPem,
                        renewalParams,
                        { initial: true, source: 'registerDevice' }
                    );
                    // Don't emit here - the manager's forwarded event handles it
                } else {
                    // Fallback: emit raw event if manager not available or info incomplete
                    this.emit('certificatesCreated', info, { initial: true, source: 'registerDevice' });
                }
            } catch (e) {
    ...
    * Also triggers onTokenCreated callback and reschedules automatic refresh.
    ...
    /**
     * Create a SIP listener for receiving doorbell notifications.
     * Requires prior device registration and certificate provisioning.
     * 
     * @param {Object} sipAccount - SIP account from deviceRegistered event or saved data
     * @param {string} sipAccount.sipUri - SIP URI (e.g., "user_clientId@gateway.bs.iotleg.com")
     * @param {string} sipAccount.sipPassword - SIP password
     * @param {string} sipAccount.plantId - Plant ID
     * @param {string} [sipAccount.gatewayId] - Gateway ID (optional)
     * @param {Object} certs - Certificates from certificatesCreated event or saved data
     * @param {string} certs.cert - Certificate PEM string
     * @param {string} certs.key - Private key PEM string
     * @param {Object} [opts] - Listener options
     * @param {boolean} [opts.debug] - Enable debug logging (inherits from auth if not specified)
     * @param {boolean} [opts.keepAlive=true] - Re-register periodically
     * @param {boolean} [opts.autoReconnect=true] - Reconnect on disconnect
     * @param {number} [opts.keepAliveInterval] - Keep-alive interval in ms (default: from config.js)
     * @param {number} [opts.reconnectDelay] - Reconnect delay in ms (default: from config.js)
     * @returns {BticinoSipListener} Configured SIP listener instance
     * @throws {Error} If sipAccount or certs are invalid
     * 
     * @example
     * // After authentication and device registration
    ...
    createSipListener(sipAccount, certs, opts = {}) {
        ...
        if (!sipAccount || typeof sipAccount !== 'object') {
            throw new Error('Invalid sipAccount: must be an object');
        }
        if (!sipAccount.sipUri || typeof sipAccount.sipUri !== 'string') {
            throw new Error('Invalid sipAccount: missing or invalid sipUri');
        }
        if (!sipAccount.sipPassword || typeof sipAccount.sipPassword !== 'string') {
            throw new Error('Invalid sipAccount: missing or invalid sipPassword');
        }

        // Validate certs
        if (!certs || typeof certs !== 'object') {
            throw new Error('Invalid certs: must be an object');
        }
        if (!certs.cert || typeof certs.cert !== 'string') {
            throw new Error('Invalid certs: missing or invalid cert');
        }
        if (!certs.key || typeof certs.key !== 'string') {
            throw new Error('Invalid certs: missing or invalid key');
        }

        // Extract username and domain from sipUri
        const username = sipAccount.sipUri.split('@')[0];
        const domain = sipAccount.sipUri.split('@')[1] || config.SIP_DOMAIN;
        
        this._log('Creating SIP listener:', {
            username,
            domain,
        ...
        // Create listener with centralized config
        const listener = new BticinoSipListener(
            {
                ...
                realm: domain,  // Use the same domain extracted from sipUri
                userAgent: `bticino-doorentry/${this._getVersion()}`
            },
            {
                certPEM: certs.cert,
            ...
            {
                debug: opts.debug !== undefined ? opts.debug : this.opts.debug,
                keepAlive: opts.keepAlive !== undefined ? opts.keepAlive : true,
                autoReconnect: opts.autoReconnect !== undefined ? opts.autoReconnect : true,
                keepAliveInterval: opts.keepAliveInterval || config.SIP_KEEPALIVE_INTERVAL,
                reconnectDelay: opts.reconnectDelay || config.SIP_RECONNECT_DELAY
            }
        );

        // Forward all SIP listener events through BticinoAuthentication for centralized event handling
        const sipEvents = [
            'connected',
            'disconnected',
            'registered',
        ...
        sipEvents.forEach(eventName => {
            listener.on(eventName, (...args) => {
                // Emit with 'sip:' prefix to distinguish SIP events from auth events
                this.emit(`sip:${eventName}`, ...args);
            });
        });

        // Integration with certificate manager for automatic certificate refresh with graceful restart
        if (this._certManager) {
            this._certManager.on('certificatesRefreshed', async (newCerts) => {
                this._log('Certificates refreshed - automatically updating listener...');
                
                try {
                    // Automatically update listener with new certificates (triggers graceful disconnect/reconnect)
                    await listener.updateCertificates({
                        cert: newCerts.cert,
                        key: newCerts.key
                    });
                    this._log('Listener successfully updated with new certificates');
                } catch (err) {
                    this._error('Failed to update listener certificates:', err.message);
                    // Error is automatically forwarded as 'sip:certificateUpdateError' event
                }
            });
        }

        // If an existing listener is present, stop it before creating a new one
        if (this._sipListener) {
            this._log('Stopping existing SIP listener instance before creating new one');
            try {
                this._sipListener.disconnect();
            } catch (_) {}
            this._sipListener = null;
        }

        // Store reference to prevent duplications
        this._sipListener = listener;

        // Track permanent closure to clear reference
        listener.on('disconnected', () => {
            if (this._sipListener === listener && listener._closing) {
                this._sipListener = null;
            }
        });
    ...
    /**
     * Stop and destroy the active SIP listener if one exists.
     * @public
     * @async
     * @returns {Promise<void>}
     * @example
     * await auth.stopSipListener();
     */
    async stopSipListener() {
        if (this._sipListener) {
            const listener = this._sipListener;
            this._sipListener = null;
            try {
                await listener.disconnect();
            } catch (err) {
                this._error('Error disconnecting SIP listener:', err && err.message);
            }
        }
    }

    /**
     * Check if a SIP listener has been created.
     * Useful to prevent creating duplicate listeners.
     * 
     * @returns {boolean} True if a listener exists (even if not connected)
     * 
     * @example
     ...
     */
    hasSipListener() {
        return this._sipListener !== null;
    }

    /**
     * Check if the SIP listener is currently connected and registered.
     * 
     * @returns {boolean} True if listener exists AND is connected AND registered
     * 
     * @example
     * if (auth.isSipListenerConnected()) {
     *   console.log('Listener is active and ready to receive calls');
     * }
     */
    isSipListenerConnected() {
        if (!this._sipListener) {
            return false;
        }
        // Check both socket connected and registered status
        return this._sipListener.socket !== null && this._sipListener.registered === true;
    }

    /**
     * Get the current SIP listener instance if one exists.
     * 
     * @returns {BticinoSipListener|null} The active listener or null if none exists
     * 
     * @example
     * const listener = auth.getSipListener();
     * if (listener) {
     *   console.log('Listener exists:', listener.isConnected);
     * }
     */
    getSipListener() {
        return this._sipListener;
    }
     ...
     */
    _log(...args) {
        if (this.opts && this.opts.debug) console.log('[BticinoAuthentication]', ...args);
    }

    /**
     * Conditional warn logger.
     * @private
     * @param {...*} args - Arguments to warn
     */
    _warn(...args) {
        if (this.opts && this.opts.debug) console.warn('[BticinoAuthentication]', ...args);
    }
     ...
     */
    _error(...args) {
        if (this.opts && this.opts.debug) console.error('[BticinoAuthentication]', ...args);
    }
