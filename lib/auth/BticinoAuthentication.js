 ...
 * @emits BticinoAuthentication#loginUrl {Object} Emitted when interactive login is required. Payload: {state, loginUrl, url}
...
class BticinoAuthentication extends EventEmitter {
    ...
    * Ensure the underlying {@link BticinoProxy} is started exactly once.
    ...
    async authenticate() {
        ...
        // the path-only URL.
        try {
            this.emit('loginUrl', { state, loginUrl: fullUrl, url: pathUrl });
        } catch (e) {
            this._log('emit(loginUrl) error:', e && e.message);
        }
