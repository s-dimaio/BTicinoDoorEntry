...
/**
 * Daily check interval for certificate renewal (24 hours in ms).
...
class BticinoCertificateManager extends EventEmitter {
     ...
     * Uses a daily polling approach instead of a single long timer because
     ...
     * 3. Otherwise, starts a daily check that monitors time until renewal
     * 
     * @private
    ...
    _scheduleCertificateRenewal(certs) {
        ...
        // Start daily polling check
        this._log('Starting daily renewal check (every 24h)');
        this._renewalTargetMs = renewAtMs;  // Store target for daily checks

        this._renewalTimer = setInterval(() => {
            this._dailyRenewalCheck();
    ...
    /**
     * Internal: daily check to see if it's time to renew.
    ...
    _dailyRenewalCheck() {
        ...
        if (remainingMs <= 0) {
            this._log('Daily check: renewal time reached, executing renewal...');
            this._clearRenewalTimer();
            this._doRenewal();
        } else {
            const daysLeft = Math.floor(remainingMs / (24 * 60 * 60 * 1000));
            this._log(`Daily check: ${daysLeft} days until certificate renewal`);
        }
    }
