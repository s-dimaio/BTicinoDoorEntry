...
class BticinoSipListener extends EventEmitter {
  ...
  _handleData(data) {
    ...
    // If in monitor mode, call the callback with ALL messages
    if (this._monitorMode && this._monitorCallback) {
      try {
        ...
        this._error('Monitor callback error:', err);
      }
    }
  ...
  /**
   * Enable monitor mode to capture ALL SIP messages.
   * When enabled, the callback will be invoked for every SIP message received.
   * 
   * @param {Function} callback - Called with {timestamp, type, method, statusCode, headers, body, raw}
   * 
   * @example
   * listener.startMonitor((msg) => {
   *   console.log('SIP Message:', msg.type, msg.method || msg.statusCode);
  ...
  startMonitor(callback) {
    if (typeof callback !== 'function') {
      throw new Error('Monitor callback must be a function');
    }
    
    this._monitorMode = true;
    this._monitorCallback = callback;
    this._log('📡 SIP Monitor mode ENABLED - capturing ALL messages');
  }
  ...
  stopMonitor() {
    this._monitorMode = false;
    this._monitorCallback = null;
    this._log('📡 SIP Monitor mode DISABLED');
  }
