...
class BticinoApiClient {
  ...
  /**
   *  **** Method still needs fixing - not working ****
   * 
   * High-level method to renew client certificates.
   * Generates new ECC P-256 key pair, creates CSR, sends to server, and returns certificate + private key.
   * @param {Object} params Certificate renewal parameters
   * @param {string} params.deviceId Device ID (CN of certificate)
   * @param {string} params.plantId Plant UUID
   * @param {string} params.gatewayId Gateway UUID (for SIP URI)
   * @param {string} params.ownerEmail Owner email address
   * @param {string} params.ownerId Owner UUID (user OID)
   * @param {string} [params.plantName] Plant name (optional)
   * @param {string} [params.country='it'] Country code
   * @returns {Promise<Object>} { certificate: string (PEM), privateKey: string (PEM), validUntil: Date }
   * @example
   * const certs = await api.renewCertificates({
   *   deviceId: 'xxxxx242xxxx03xxxx63',
   *   plantId: 'exxxxe-xxx9-xxxx4-9xxxxc-7xxxxxx63',
   *   gatewayId: '318xxxx-xxxx-xxx-xx-161xxxxba4xxx2',
   *   ownerEmail: 'user@example.com',
   *   ownerId: 'bexxxx9a-xxx-xxx-xxx-8ax6xxxx5xx'
   * });
   * fs.writeFileSync('client-cert.pem', certs.certificate);
   * fs.writeFileSync('client-key.pem', certs.privateKey);
   */
  async renewCertificates(params) {
    const { deviceId, plantId, gatewayId, ownerEmail, ownerId, plantName = 'Home', country = 'it' } = params;
    ...
    const csrBase64 = this._createCSR(privateKey, subject, sipUri);
    ...
    const response = await this.provisionClientCertificate(requestBody);

    // 4. Parse certificate and extract validity
  ...
  /**
   * Create a Certificate Signing Request (CSR) in DER format (base64 encoded).
   * @private
   * @param {string} privateKeyPEM Private key in PEM format
   * @param {Array} subject Array of {name, value} for certificate subject
   * @param {string} sipUri SIP URI for Subject Alternative Name
   * @returns {string} Base64-encoded CSR (DER format)
   */
  _createCSR(privateKeyPEM, subject, sipUri) {
    const { execSync } = require('child_process');
    const tmp = require('os').tmpdir();

    // Write private key to temp file
    const tmpKeyPath = `${tmp}/bticino_key_${Date.now()}.pem`;
    fs.writeFileSync(tmpKeyPath, privateKeyPEM);

    // Create OpenSSL config
    const configPath = `${tmp}/bticino_csr_${Date.now()}.conf`;
    const opensslConfig = `
[req]
default_bits = 256
...
CN = ${subject.find(s => s.name === 'commonName').value}
OU = ${subject.find(s => s.name === 'organizationalUnitName').value}
O = ${subject.find(s => s.name === 'organizationName').value}
L = ${subject.find(s => s.name === 'localityName').value}
ST = ${subject.find(s => s.name === 'stateOrProvinceName').value}
C = ${subject.find(s => s.name === 'countryName').value}
emailAddress = ${subject.find(s => s.name === 'emailAddress').value}

[v3_req]
subjectAltName = URI:${sipUri}
`;

    fs.writeFileSync(configPath, opensslConfig.trim());

    // Generate CSR with OpenSSL
    const tmpCsrPath = `${tmp}/bticino_csr_${Date.now()}.csr`;

    try {
      execSync(
        `openssl req -new -key "${tmpKeyPath}" -out "${tmpCsrPath}" -config "${configPath}" -outform DER`,
        { stdio: 'pipe' }
      );

      const csrDER = fs.readFileSync(tmpCsrPath);
      const csrBase64 = csrDER.toString('base64');

      // Cleanup
      fs.unlinkSync(tmpKeyPath);
      fs.unlinkSync(configPath);
      fs.unlinkSync(tmpCsrPath);
