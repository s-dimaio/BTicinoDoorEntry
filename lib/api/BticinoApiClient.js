...
class BticinoApiClient {
  ...
  async _get(url) {
    ...
    this._log && this._log(`📡 HTTP GET: ${url}`);
    
    try {
      const res = await axios.get(url, { headers: this._headers(), timeout: this.opts.timeoutMs });
      
      this._log && this._log(`✅ HTTP ${res.status} response received`);
      this._log && this._log(`  - Response type: ${typeof res.data}`);
      this._log && this._log(`  - Is Array: ${Array.isArray(res.data)}`);
      if (Array.isArray(res.data)) {
        this._log && this._log(`  - Array length: ${res.data.length}`);
      }
      this._log && this._log(`  - Response preview (first 500 chars): ${JSON.stringify(res.data).substring(0, 500)}`);
      
      return res.data;
    } catch (error) {
      this._error(`❌ HTTP GET failed for ${url}`);
      this._error(`  - Error: ${error.message}`);
      if (error.response) {
        this._error(`  - Status: ${error.response.status}`);
        this._error(`  - Response data: ${JSON.stringify(error.response.data)}`);
      }
      throw error;
    }
  }
